...
class EmergAlertBot (BotBase):
    ...
    def start_data(self, event):
        ...
        txt1 = ''
        # Сделать проверку, что пользователь заведен в БД!!!
        if 1 == 0:
            pass
        elif alert:
            ...
            if not Response:
                txt = f'{fullname}, Вы не участвуете в текущем оповещении, т.к. не было предоставлено информации по Вам! Администраторы бота свяжутся с Вами.'
                txt1 = ''
                self.bot.send_text(chat_id='moseevay@veb.ru', text = '@[' + userId + '] не может получить оповещение. Необходимо внести данные пользователя.', parse_mode='HTML')
                # отправить оповещение Админам бота
            elif Response != 'Нет ответа' or len(alert['responses']) == 0:
            ...
            if len(roles) > 1 and (self.db_manager.alert_get_alert_response(userId, alert['_id']) != 'Нет ответа' or len(alert['responses']) == 0):
    ...
    def message_cb(self, bot, event):
        if not event.text.startswith('/') and event.data['chat']['type'] == "private":
            ...
            if last_press[0] == 'Создать оповещение' and 'Администратор' in roles:
                ...
                data = self.xlsx_url_to_dict(file_url)
    ...
    def xlsx_to_json(self, xlsx_file, json_file):
        ...
        with open(json_file, 'w', encoding='utf-8') as f:
            f.write(json_data)
    # xlsx url to dict
    def xlsx_url_to_dict(self, url):
        # Загрузка файла по URL
        response = requests.get(url)
        response.raise_for_status()  # Проверка на ошибки при загрузке
        # Чтение файла Excel из байтового потока
        df = pd.read_excel(BytesIO(response.content))
        # Замена NaN на пустые строки
        df = df.fillna('')
        # Преобразование DataFrame в словарь
        data_dict = df.to_dict(orient='records')  # 'records' создаёт список словарей
        return data_dict
if __name__ == "__main__":
    bot_app = EmergAlertBot()
    bot_app.run()
    #   asyncio.run(bot_app.some_method(alert))
