...
class Character(BaseSprite):
    ...
    def check_dir(self):
        ...
        if self.old_direction != self.direction:
            if self.direction > 0:
                ...
                self.image = game.costumes[
                    self.graph_index + gr_total
    ...
    def update(self):
        ...
        if self.stands_on:
            if (
                self.y_speed
                < 0  # оттолкнулись от платформы и ушли на y_speed относительно неё
                or self.rect.right
                < self.stands_on.rect.left  # оказались левее левого края опоры
