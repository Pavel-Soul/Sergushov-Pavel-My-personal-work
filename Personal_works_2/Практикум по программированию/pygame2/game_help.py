...
class Help:
    def __init__(self):
        ...
        self.small_font = pygame.font.Font(
            None, 24
        )  # этот шрифт надо помнить и для постоянной надписи
        text1 = big_font.render(" Управление: ", 1, C_WHITE)
        text2 = self.small_font.render(
            " Движение и прыжки: стрелки или 'ADW'", 1, C_WHITE
        )
        text3 = self.small_font.render(" Выстрел: пробел ", 1, C_WHITE)
        text4 = self.small_font.render(
            " Музыка вкл/выкл: 'M', громче: 'U', тише: ' J ' ", 1, C_WHITE
        )
        text5 = self.small_font.render(
            " Подсказка вкл/выкл (игра на паузе): 'H' ", 1, C_WHITE
        )
    ...
    def line(self, points=0, lives=1):
        tab = 50
        img = pygame.Surface(
            [win_width, self.text_height], pygame.SRCALPHA
        )  # исходник - попиксельно прозрачный
        ...
        img.blit(
            self.text_help, (self.text_lives_w + tab + self.text_points_w + tab, 0)
        )
        return img
