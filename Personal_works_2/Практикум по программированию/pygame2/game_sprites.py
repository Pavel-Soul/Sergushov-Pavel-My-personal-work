...
class Actor(Character):
    ...
    def fire(self, img, is_Enemy):
        rect = (
            img.get_rect()
        )  # приложим прямоугольник для снаряда с нужной стороны того, кто стреляет:
        rect.y = (
            self.rect.centery - 15 + randint(-3, 3)
        )  # это можно менять в зависимости от соотношения картинок
        if self.direction > 0:
            rect.left = self.rect.right
            area = pygame.Rect(
                rect.left, rect.top, FIRE_DISTANCE, win_height - rect.top
            )
            f_speed = FIRE_SPEED
        else:
            rect.right = self.rect.left
            area = pygame.Rect(
                rect.right - FIRE_DISTANCE,
                rect.top,
                FIRE_DISTANCE,
                win_height - rect.top,
            )
        ...
        if is_Enemy:
            game.add_enemy(
                Fire(img, rect.x, rect.y, area, f_speed, graph_index=gr_enemyfire)
            )  # создаем снаряд с вычисленными параметрами
        # и прописываем как врага
        else:
            game.add_fire(
                Fire(img, rect.x, rect.y, area, f_speed)
            )  # создаем снаряд с вычисленными параметрами
...
class Enemy(Actor):
    ...
    def fire(self):
        img = game.costumes[
            gr_enemyfire
        ]  # generate_image(1, 1, C_WHITE) # тут нужна картинка для снаряда врага
        super().fire(img, True)
...
class Hero(Actor):
    ...
    def fire(self):
        img = game.costumes[
            gr_fire
        ]  # generate_image(1, 1, C_DARK) # тут нужна картинка для снаряда героя
